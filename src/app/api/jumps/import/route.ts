...
export async function POST(req: NextRequest) {
  try {
    ...
    // Ensure user record exists before import (prevents "Unique constraint failed" error)
    let userRecord = await prisma.user.findUnique({ where: { id: user.id } })

    if (!userRecord) {
      // Create user if doesn't exist - handles race condition
    ...
    const [dropzones, aircrafts, jumpTypes] = await Promise.all([
